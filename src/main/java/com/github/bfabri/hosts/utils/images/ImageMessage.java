package com.github.bfabri.hosts.utils.images;
...
public class ImageMessage {
    ...
    public void sendToPlayers(ArrayList<GamePlayer> gamePlayers) {
        for (String line : lines) {
	        gamePlayers.forEach(gamePlayer -> gamePlayer.getPlayer().sendMessage(Utils.translate(line)));
        }
    }

    public void sendToAllPlayers() {
        for (String line : lines) {
            TextComponent message = new TextComponent(Utils.translate(line));
            if (line.contains(ConfigHandler.Configs.LANG.getConfig().getString("ANNOUNCEMENTS.START.EVENT-ACTIVATOR"))) {
                message.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
                        (new ComponentBuilder(Utils.translate(ConfigHandler.Configs.LANG.getConfig().getString("ANNOUNCEMENTS.START.HOVER")))).create()));
