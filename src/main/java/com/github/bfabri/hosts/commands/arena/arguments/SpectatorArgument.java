...
public class SpectatorArgument extends CommandArgument {
	public SpectatorArgument() {
		super("spectator", "Set spectator spawn for an arena");
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (arena != null) {
			...
			if (arenaCuboid.contains(((Player) sender).getLocation())) {
				arena.setSpectatorLocation(((Player) sender).getLocation().serialize());
				sender.sendMessage(Utils.PREFIX + Utils.translate("&eSpectator location for arena &7" + args[2] + " &ehas been set!"));
				Hosts.getInstance().getArenaManager().saveArenas();
			} else {
				sender.sendMessage(Utils.PREFIX + Utils.translate("&cYou have to set the spectator inside the arena."));
			}
		} else {
